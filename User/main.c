...
static void TIM1_Config(void){
	...
	/*
	 * pulse is the CCR register in OC mode
	 * pulse dicates the maximum pulse width that can be loaded into the OC registers
	 * 1000 in OC register will get compared with 1000 in CCR to produce 100% pulse width for PWM
	 */
 ...
 * motor encoder on motor used has 2k counts/rev
 */
static void Encoder_Config(void){
	GPIO_InitTypeDef GPIO_InitStruct;
 ...
 * This will call functions to get data from MPU6050 and do integration to get angular velocity
...
void Process_Data(void){
	...
	if (MPU6050_Read(I2C1,MPU6050_Device_0,MPU6050_INT_STATUS,0)){
		...
		if(drift>3 || drift<-3)
		{
			...
			//do discrete integration
			//TODO: Find more optimal method for discrete integration
...
/*
 * Hall effect signals are used to indicate rotor position
 * and to process required phase position for stator
 * process hall will signal to L6234 to phases on and off independently of the rest of the system
 * the 3 phase motor should eventually sync hall effect signal with communcation signal
 * it may be possible to turn this off during steady state operation...
 */
void Process_Hall(void){
	//Convert the reading into int
	uint8_t temp;

	//TODO: use bit shifting here
